...
func TestEffectiveClientLimit_UsesDeviceLimitWhenStricter(t *testing.T) {
	client := model.Client{LimitIP: 3, DeviceLimit: 1}

	if got := effectiveClientLimit(client); got != 1 {
		t.Fatalf("expected device limit 1 to be effective, got %d", got)
	}
}

func TestEffectiveClientLimit_UsesDeviceLimitWhenIPLimitDisabled(t *testing.T) {
	client := model.Client{LimitIP: 0, DeviceLimit: 2}

	if got := effectiveClientLimit(client); got != 2 {
		t.Fatalf("expected device limit 2 to be effective, got %d", got)
	}
}

func TestEffectiveClientLimit_KeepsIPLimitWhenDeviceLimitDisabled(t *testing.T) {
	client := model.Client{LimitIP: 2, DeviceLimit: 0}

	if got := effectiveClientLimit(client); got != 2 {
		t.Fatalf("expected IP limit 2 to be effective, got %d", got)
	}
}
