...
func (j *CheckClientIpJob) hasLimitIp() bool {
	...
	for _, inbound := range inbounds {
		...
		for _, client := range clients {
			limitIp := effectiveClientLimit(client)
			if limitIp > 0 {
				return true
			}
		}
...
func effectiveClientLimit(client model.Client) int {
	limit := client.LimitIP
	if client.DeviceLimit > 0 && (limit <= 0 || client.DeviceLimit < limit) {
		limit = client.DeviceLimit
	}
	return limit
}
