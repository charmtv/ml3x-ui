...
fetch_latest_release_tag() {
    local repo=$1
    local curl_opt=$2
    curl ${curl_opt} -Ls "https://api.github.com/repos/${repo}/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/'
}
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        curl -4fLRo ${xui_folder}-linux-$(arch).tar.gz https://github.com/${release_repo}/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        url="https://github.com/${primary_release_repo}/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz"
        echo -e "Beginning to install x-ui $1"
        curl -4fLRo ${xui_folder}-linux-$(arch).tar.gz ${url}
        if [[ $? -ne 0 ]]; then
            echo -e "${yellow}Download from ${primary_release_repo} failed, trying upstream release...${plain}"
            url="https://github.com/${fallback_release_repo}/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz"
            curl -4fLRo ${xui_folder}-linux-$(arch).tar.gz ${url}
            if [[ $? -ne 0 ]]; then
    ...
    curl -4fLRo /usr/bin/x-ui-temp https://raw.githubusercontent.com/charmtv/ml3x-ui/main/x-ui.sh
    if [[ $? -ne 0 ]]; then
    ...
    if [[ $release == "alpine" ]]; then
        curl -4fLRo /etc/init.d/x-ui https://raw.githubusercontent.com/charmtv/ml3x-ui/main/x-ui.rc
        if [[ $? -ne 0 ]]; then
        ...
        if [ "$service_installed" = false ]; then
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/charmtv/ml3x-ui/main/x-ui.service.debian >/dev/null 2>&1
                ;;
                arch | manjaro | parch)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/charmtv/ml3x-ui/main/x-ui.service.arch >/dev/null 2>&1
                ;;
                *)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/charmtv/ml3x-ui/main/x-ui.service.rhel >/dev/null 2>&1
                ;;
            esac
