...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/charmtv/ml3x-ui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    confirm "此功能会将 x-ui 更新到最新版本，数据不会丢失，是否继续？" "y"
    if [[ $? != 0 ]]; then
        LOGE "已取消"
        if [[ $# == 0 ]]; then
            before_show_menu
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/charmtv/ml3x-ui/main/update.sh)
    if [[ $? == 0 ]]; then
        LOGI "更新完成，面板已自动重启"
        before_show_menu
    fi
...
update_menu() {
    ...
    confirm "此功能会将服务器菜单更新到最新版，是否继续？" "y"
    if [[ $? != 0 ]]; then
        LOGE "已取消"
        if [[ $# == 0 ]]; then
            before_show_menu
    ...
    curl -fLRo /usr/bin/x-ui https://raw.githubusercontent.com/charmtv/ml3x-ui/main/x-ui.sh
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls https://raw.githubusercontent.com/charmtv/ml3x-ui/v$tag_version/install.sh) v$tag_version"
...
uninstall() {
    ...
    echo ""
    echo -e "卸载成功。\n"
    echo "如果需要重新安装，请使用以下命令："
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/charmtv/ml3x-ui/main/install.sh)${plain}"
    echo ""
...
reset_user() {
    ...
    read -rp "是否禁用当前已配置的二次验证？(y/n): " twoFactorConfirm
    ...
    echo -e "${green}请使用新的用户名和密码登录面板，并妥善保存！${plain}"
    confirm_restart
}
    ...
    if [[ -n "$existing_cert" ]]; then
        ...
        if [[ "$gen_ssl" == "y" || "$gen_ssl" == "Y" ]]; then
            ...
            if [[ $? -eq 0 ]]; then
                ...
                LOGE "IP 证书配置失败。"
                echo -e "${yellow}你可以通过菜单 19（SSL 证书管理）重试。${plain}"
                start >/dev/null 2>&1
            fi
        else
            echo -e "${yellow}访问地址：http://${server_ip}:${existing_port}${existing_webBasePath}${plain}"
            echo -e "${yellow}为了安全，请通过菜单 19（SSL 证书管理）配置证书。${plain}"
        fi
    fi
}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        if [[ $? == 0 ]]; then
            ...
            LOGE "面板启动失败，可能启动耗时超过 2 秒，请稍后检查日志。"
        fi
    fi
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        if [[ $? == 1 ]]; then
            ...
            LOGE "面板停止失败，可能停止耗时超过 2 秒，请稍后检查日志。"
        fi
    fi
...
restart() {
    ...
    if [[ $? == 0 ]]; then
        ...
        LOGE "面板重启失败，可能启动耗时超过 2 秒，请稍后检查日志。"
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
restart_xray() {
    ...
    LOGI "Xray 重启信号已发送，请查看日志确认是否重启成功。"
    sleep 2
    show_xray_status
    if [[ $# == 0 ]]; then
        before_show_menu
...
disable_bbr() {
    ...
    if [[ $(sysctl -n net.ipv4.tcp_congestion_control) != "bbr" ]]; then
        ...
        echo -e "${red}切换 BBR 到 CUBIC 失败，请检查系统配置。${plain}"
    fi
}
...
update_shell() {
    curl -fLRo /usr/bin/x-ui -z /usr/bin/x-ui https://github.com/charmtv/ml3x-ui/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
        LOGE "脚本下载失败，请检查服务器是否能连接 GitHub"
        before_show_menu
    else
        chmod +x /usr/bin/x-ui
        LOGI "脚本升级成功，请重新运行 x-ui"
        before_show_menu
    fi
}
...
firewall_menu() {
    ...
    echo -e "${green}\t2.${plain} 端口规则列表 [带编号]"
    echo -e "${green}\t3.${plain} ${green}放行${plain}端口"
...
open_ports() {
    ...
    if ! [[ $ports =~ ^([0-9]+|[0-9]+-[0-9]+)(,([0-9]+|[0-9]+-[0-9]+))*$ ]]; then
        echo "错误：输入无效，请输入逗号分隔的端口或端口范围（例如 80,443,2053 或 400-500）。" >&2
        exit 1
    fi
...
update_geo() {
    ...
    1)
        update_geofiles "main"
        echo -e "${green}Loyalsoldier 数据文件已成功更新！${plain}"
        restart
        ;;
    2)
        update_geofiles "IR"
        echo -e "${green}chocolate4u 数据文件已成功更新！${plain}"
        restart
        ;;
    3)
        update_geofiles "RU"
        echo -e "${green}runetfreedom 数据文件已成功更新！${plain}"
        restart
        ;;
    4)
        update_all_geofiles
...
ssl_cert_issue_main() {
    ...
    echo -e "${green}\t6.${plain} 为 IP 申请 SSL（6 天证书，自动续期）"
    echo -e "${green}\t0.${plain} 返回主菜单"
    ...
    5)
        ...
        ;;
    6)
        echo -e "${yellow}为 IP 申请 Let's Encrypt SSL 证书${plain}"
        echo -e "这将使用短期证书配置为你的服务器 IP 申请证书。"
        echo -e "${yellow}证书有效期约 6 天，将通过 acme.sh 定时任务自动续期。${plain}"
        echo -e "${yellow}端口 80 必须开放，并且公网可访问。${plain}"
        confirm "是否继续？" "y"
        if [[ $? == 0 ]]; then
            ssl_cert_issue_for_ip
        fi
...
ssl_cert_issue_for_ip() {
    ...
    LOGI "使用 Let's Encrypt 短期证书配置（约 6 天有效，自动续期）"
    ...
    *)
    ...
    LOGI "将使用端口 ${WebPort} 为 IP 签发证书：${server_ip}"
    if [[ "${WebPort}" -ne 80 ]]; then
        LOGI "提醒：Let's Encrypt 仍会访问 80 端口，请将外部 80 端口转发到 ${WebPort} 用于验证。"
    fi
    ...
    while true; do
        if is_port_in_use "${WebPort}"; then
            ...
            local alt_port=""
            read -rp "请输入 acme.sh 独立监听的其它端口（留空中止）：" alt_port
            alt_port="${alt_port// /}"
            if [[ -z "${alt_port}" ]]; then
                LOGE "端口 ${WebPort} 被占用，无法继续签发证书。"
                return 1
            fi
    ...
    *)
    ...
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        ...
        1)
            ...
            ;;
        2)  
            LOGD "建议把 x-ui restart 放在最后，避免其它服务失败影响面板重启"
            read -rp "请输入 reloadcmd（例如：systemctl reload nginx ; x-ui restart）：" reloadCmd
...
ssl_cert_issue_CF() {
    ...
    if [ $? -eq 0 ]; then
        ...
        ~/.acme.sh/acme.sh --set-default-ca --server letsencrypt --force
        if [ $? -ne 0 ]; then
            LOGE "设置默认证书机构 Let's Encrypt 失败，脚本退出..."
            exit 1
        fi
    ...
    if ! command -v speedtest &>/dev/null; then
        ...
        if command -v snap &>/dev/null; then
            ...
            if [[ -z $pkg_manager ]]; then
                ...
                echo "正在使用以下包管理器安装 Speedtest：$pkg_manager..."
...
install_iplimit() {
    if ! command -v fail2ban-client &>/dev/null; then
        ...
        alpine)
            ...
            ;;
        *)
            echo -e "${red}不支持的操作系统，请检查脚本并手动安装必要组件。${plain}\n"
            exit 1
            ;;
        esac
...
show_banlog() {
    ...
    if [[ -f "$system_log" ]]; then
        echo -e "${green}fail2ban.log 中最近的系统封禁记录：${plain}"
        grep "3x-ipl" "$system_log" | grep -E "Ban|Unban" | tail -n 10 || echo -e "${yellow}未找到最近的系统封禁记录${plain}"
        echo ""
    fi
...
EOF
    ...
    echo -e "${green}IP 限制 jail 文件已创建，封禁时长：${bantime} 分钟。${plain}"
}
...
│  ${blue}x-ui update-all-geofiles${plain}   - 更新 Geo 文件                    │
│  ${blue}x-ui legacy${plain}                - 安装指定版本                     │
